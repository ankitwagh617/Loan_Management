...
class Connection:
    def __init__(self):
        self.con = pm.connect(host='localhost',user='root',password='1234',database='try')
    ...
    def getallloaninfo(self,id):
        sql = "select * from loaninfo where agent_info = '%s' " % (id)
        self.cursor.execute(sql)
        self.hoil = []
    ...
    def acceptedloans(self,lid,aid):
        sql = "select * from loaninfo where loan_id  = '%d'" % (lid)
        self.cursor.execute(sql)
        self.hoil = []
        self.hoil = self.cursor.fetchone()
        print(self.hoil)
