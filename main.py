...
def agentuser():
    if 'agent_user' in session:
        ...
        data = []
        print(agent_user)
        data = cnn.getallloaninfo(agent_user)
            # data.insert(0,emp)
        print(data)
        headings = ("LOAN ID","LOAN AMOUNT",  "LOAN TYPE","INTEREST RATE", "COLLATERAL VALUE","EMAIL","AGENT ID")
        return render_template('demo.html',headings =  headings, data = data)
...
@app.route('/addloandata',methods=['GET','POST'])
def addloandata():
    if request.method == 'POST':
        id = int(request.form['uacc'])
        email = session['email']
        agent_user = session['agent_user']
        print(id)
        print(type(id))
        ...
        data = []
        data = cnn.getallloaninfo(agent_user)
        if cnn.acceptedloans(id,agent_user) == True:
            return render_template('demo.html',headings = headings,data=data)
